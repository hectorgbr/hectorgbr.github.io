...
// The addItem method uses an efficient algorithm to either insert a new item or update
// the count of an existing item. This ensures that each item is only stored once per user
// and the count is incremented as needed.
...
// Function to delete an item from the user's account
bool Database::deleteItem(const std::string& username, const std::string& item) {
    // SQL query to delete the item
    std::string query = "DELETE FROM items WHERE username = '" + username + "' AND item = '" + item + "'";
    try {
        executeQuery(query);
        return true;
    }
